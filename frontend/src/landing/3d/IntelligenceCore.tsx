...
export function IntelligenceCore({ activeStage = 'hero', scrollProgress = 0 }: { activeStage?: string; scrollProgress?: number }) {
  const meshRef = useRef<THREE.Group>(null);
  const ringRef = useRef<THREE.Mesh>(null);

  const rotationSpeed = useMemo(() => 0.5 + scrollProgress * 2, [scrollProgress]);

  const coreOpacity = 0.85;
  ...
  return (
    <group>
      <group ref={meshRef} position={[0, -0.65, -0.5]} scale={0.82}>
          <mesh position={[0, 0, 0]}>
