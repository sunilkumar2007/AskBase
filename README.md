...
### Unique System Commands

| Command Type | Syntax / Example | Action & Description |
|---|---|---|
| **Table Targeting** | `@sales`, `@customers`, `@orders` | Forces the AI to strictly search within those specific database tables, guaranteeing 100% precision and eliminating hallucination. |
| **Table Targeting** | `@products`, `@revenue` | Explicitly restricts the LLM query context exclusively to the specified data schemas. |
...
| **Help System** | `/help` | Displays available slash and `@` table commands. |
