...
class TestSTTProvider:
	...
	async def test_gemini_stt_transcribe(self):
		...
		gemini = await provider._get_gemini()

		with patch.object(type(gemini), "enabled", new_callable=PropertyMock, return_value=True):
			with patch.object(gemini, "generate_structured", new_callable=AsyncMock) as mock_gen:
...
class TestVoiceService:
	...
	async def test_process_voice_request_success(self):
		...
		with patch.object(service._stt, "transcribe", new_callable=AsyncMock) as mock_stt:
			...
			with patch("app.voice.service.Agent", return_value=mock_agent):
				with patch("app.voice.service.AgentContext") as mock_ctx_cls:
					mock_ctx = MagicMock()
